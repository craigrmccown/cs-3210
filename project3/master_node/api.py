...
def confirm_registration(slave_node_id, slave_node_port_number):
    ...
    for node in cursor:
        requests.post(''.join(['http://', node.get('ip_address'), ':', str(node.get('port_number')), '/topology']), new_node)

    db.topology.insert(new_node)


def generate_v_nodes(num_v_nodes):
    v_nodes = []
